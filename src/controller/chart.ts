...
export default class ChartController {
    ...
    constructor(
        ...
        this.router.get("/traffic", this.handleTrafficChartReq)
    }

    private readonly handleTrafficChartReq: RequestHandler = async (_, res) => {
        const logName = `${this.constructor.name}:handleTrafficChartReq`

        console.debug(`${logName} :: quering database`)
        const stats = await this.db.getDailyPeriodStats()

        console.debug(`${logName} :: constructing chart`)
        const trafficChart = constructTrafficBarChart(stats, this.groups, this.defaultGroup)

        console.debug(`${logName} :: sending response`)
        res.json(trafficChart)
    }
}
